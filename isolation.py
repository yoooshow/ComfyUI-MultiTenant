...
def sync_za_workflow_to_users(workflow_data: dict, display_name: str) -> None:
    """Write a Z&A workflow JSON into every user's workflows dir.
    ...
    try:
        ...
        for user_id in _all_mt_user_ids():
            ...
            dest = os.path.join(wf_dir, f"{safe_name}.json")
            with open(dest, "w", encoding="utf-8") as f:
                f.write(content)
        logger.info(f"[ComfyUI-MT] Z&A workflow '{display_name}' synced to {len(_all_mt_user_ids())} users")
    except Exception as e:
        logger.error(f"[ComfyUI-MT] Z&A sync failed: {e}")


def remove_za_workflow_from_users(display_name: str) -> None:
    """Remove a Z&A workflow JSON from every user's workflows dir."""
    try:
        safe_name = "".join(c for c in display_name if c not in '/\\:*?"<>|').strip() or "workflow"
        for user_id in _all_mt_user_ids():
            wf_dir = _workflows_dir_for_user(user_id)
            ...
            if os.path.isfile(dest):
                os.remove(dest)
        logger.info(f"[ComfyUI-MT] Z&A workflow '{display_name}' removed from users")
    except Exception as e:
