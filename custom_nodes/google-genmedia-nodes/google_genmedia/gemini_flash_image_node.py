...
class Gemini25FlashImage:
            ...
            "required": {
                "model": (
                    ...
                    {"default": "gemini-2.5-flash-image-preview",
                     "tooltip": "Model name, e.g. gemini-2.5-flash-image"},
                ),
                "prompt": (
                    "STRING",
                    {
    ...
    def generate_and_return_image(
        ...
        if aspect_ratio != "auto" and aspect_ratio not in GEMINI_25_FLASH_IMAGE_ASPECT_RATIO:
            raise RuntimeError(
        ...
        debug_info = ""
        if debug:
            log_path = getattr(gemini_flash_image_api, 'last_log_path', '')
            debug_info = f"Debug log: {log_path}" if log_path else "Debug log: (file not created)"
...
class Gemini31FlashImage:
            ...
            "required": {
                "model": (
                    ...
                    {"default": "gemini-3.1-flash-image-preview",
                     "tooltip": "Model name, e.g. gemini-3.1-flash-image-preview"},
                ),
                "prompt": (
                    "STRING",
                    {
                ...
                "aspect_ratio": (
                    ["auto"] + GEMINI_31_FLASH_IMAGE_ASPECT_RATIO,
                    {"default": "16:9"},
                ),
                "image_size": (
                    [
    ...
    def generate_and_return_image(
        ...
        if aspect_ratio != "auto" and aspect_ratio not in GEMINI_31_FLASH_IMAGE_ASPECT_RATIO:
            raise RuntimeError(
...
NODE_DISPLAY_NAME_MAPPINGS = {
    "Gemini25FlashImage": "Gemini 2.5 Flash Image (\U0001f34c)",
    "Gemini31FlashImage": "Gemini 3.1 Flash Image (\U0001f34c)",
    }
