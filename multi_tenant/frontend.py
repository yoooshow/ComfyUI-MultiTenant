...
<div class="navbar">
  ...
  <div class="user-info">
    <span class="bal" id="nav-bal">Token: ---</span>
...
function renderStats(el){
  api('GET','/api/admin/stats').then(function(d){
    el.innerHTML='<div class="stats-grid"><div class="stat-card"><div class="num">'+d.total_users+'</div><div class="lbl">用户数</div></div><div class="stat-card"><div class="num">'+d.total_transactions+'</div><div class="lbl">交易数</div></div><div class="stat-card"><div class="num">'+d.total_tokens_consumed.toLocaleString()+'</div><div class="lbl">消耗Token</div></div></div>';
  }).catch(function(e){el.innerHTML='<div class="empty">加载失败: '+e.message+'</div>'});
}
...
if(!token){location.href='/'}else{
  (function(){fetch('/api/auth/me',{headers:{'Authorization':'Bearer '+token}}).then(function(r){return r.json()}).then(function(u){
    ...
    renderTab('users');
    // Update balance
    fetch('/api/users/me/balance',{headers:{'Authorization':'Bearer '+token}}).then(function(r){return r.json()}).then(function(d){document.getElementById('nav-bal').textContent='Token: '+d.token_balance.toLocaleString()})
