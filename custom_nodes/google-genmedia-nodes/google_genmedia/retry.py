        ...
        while retries <= retry_count:
            try:
                ...
                if status_code == 400:
                    ...
                    raise APIExecutionError(
                        f"API key authentication failed (HTTP {status_code}). "
                        f"Check your api_key and model name. Error: {details}"
                    ) from e
            ...
            except (ssl.SSLError, httpcore.ConnectError, ConnectionError, TimeoutError, socket.gaierror, OSError) as e:
                if retries < retry_count:
                    logger.warning(
                        f"Network/SSL error (attempt {retries+1}/{retry_count}): {e}"
                    )
                    ...
                    raise APIExecutionError(
                        f"Network/SSL error after {retry_count} attempts: {e}"
                    ) from e
